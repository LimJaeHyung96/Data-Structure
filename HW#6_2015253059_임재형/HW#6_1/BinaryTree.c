...
void PreorderTraverse(BinTree* bt, visitFuncPtr action) {
	if (bt != NULL) {
		action(bt->data); //출력
		PreorderTraverse(bt->left, action); //왼쪽 서브트리 방문
		PreorderTraverse(bt->right, action); //오른쪽 서브트리 방문
	}
}
...
void PostorderTraverse(BinTree* bt, visitFuncPtr action) {
	if (bt != NULL) {
		PostorderTraverse(bt->left, action); //왼쪽 서브트리 방문
		PostorderTraverse(bt->right, action); //오른쪽 서브트리 방문
		action(bt->data); //출력
	}
}
